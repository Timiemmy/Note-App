...
@receiver(post_delete, sender=Note) # the signal is triggered after a delete() function in the Post model
...
@receiver(pre_save, sender=Note) # the signal is triggered before a save() function in the Post model
def updating_image_delete(sender, instance, **kwargs): # taking the model and the specific post (instance) as parameters
    if instance.pk:
        ...
        try:
            old_image = Note.objects.get(pk=instance.pk).image
...
def updating_audio_delete(sender, instance, **kwargs):
    if instance.pk:
        try:
            old_audio = Note.objects.get(pk=instance.pk).audio
        except Note.DoesNotExist:
            return False
        else:
